# Tilde installer for Windows.
# Run in PowerShell:  powershell -ExecutionPolicy Bypass -File install.ps1

$Here = Split-Path -Parent $MyInvocation.MyCommand.Definition
$BinDir = Join-Path $env:LOCALAPPDATA "tilde\bin"

# 1. Python check.
$py = Get-Command python -ErrorAction SilentlyContinue
if (-not $py) { $py = Get-Command python3 -ErrorAction SilentlyContinue }
if (-not $py) {
    Write-Host "Tilde needs Python. Install it from python.org, then run this again."
    exit 1
}

# 2. Put a launcher on PATH.
New-Item -ItemType Directory -Force -Path $BinDir | Out-Null
$launcher = Join-Path $BinDir "tilde.cmd"
...
Write-Host ""
Write-Host "Then open a new terminal and run:  tilde welcome"
